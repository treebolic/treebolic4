...
public class OwlModelFactory implements ImageDecorator
{
    ...
    public Tree makeTree(final String urlString)
    {
        ...
        if (classIri != null || classShortForm != null)
        {
            ...
            if (clazz != null)
            {
                if (target != null)
                {
                    ...
                    @NonNull String id = clazz.getLocalName();
                    @NonNull String name = id;
                    @NonNull String label = clazz.label != null ? clazz.label : name;
        ...
        else
        {
            ...
            {
                Ontology.Class rootClass = null;
                for (Iterator<Ontology.Class> it = tops.iterator(); it.hasNext(); )
                {
                    @NonNull Ontology.Class top = it.next();
                    if (top.iri.endsWith("#Thing"))
                    {
                        rootClass = top;
                        it.remove();
                        break;
                ...
                if (rootClass == null)
                {
                    rootClass = ontology.createClass("#Thing");
                }
    ...
    public TreeMutableNode visitClass(final INode parentClassNode, @NonNull final Ontology.Class clazz, final String ontologyUrl)
    {
        ...
        @NonNull final String name = id;
        @Nullable final String label = clazz.label != null ? clazz.label : name;
        ...
        classNode.setLabel(label + (isRelation ? "\nRelation" : ""));
        classNode.setTarget(id);
    ...
    public void visitInstances(@NonNull final TreeMutableNode parentNode, @NonNull final Stream<Ontology.Thing> things)
    {
        @NonNull final List<INode> childNodes = things //
                ...
                .map(thing -> {
                    ...
                    @NonNull final String instanceId = thing.getLocalName();
                    @NonNull final String instanceName = instanceId;
                    @NonNull final String instanceLabel = thing.label != null ? thing.label : instanceName;
                    ...
                    @NonNull final MutableNode instanceNode = new MutableNode(null, instanceName);
                    instanceNode.setLabel(instanceLabel);
                    instanceNode.setTarget(instanceId);
    ...
    {
        final String relationSubtype = property.subtype == null ? "" : "\n" + property.subtype;
        @NonNull final String relationId = property.getLocalName();
        @NonNull final String relationName = relationId + relationSubtype;
        @NonNull final String relationLabel = property.label != null ? property.label + relationSubtype : relationName;

        @NonNull final MutableNode relationNode = new MutableNode(parentNode, relationName);
        relationNode.setLabel(relationLabel);
        relationNode.setTarget(relationId);
        ...
        {
            @NonNull final MutableNode domainsNode = new MutableNode(relationNode, relationName + "-domains");
            domainsNode.setLabel("domains");
            decorateRelation(domainsNode);
            domains.forEach(domainClass -> {

                @NonNull final String domainId = domainClass.getLocalName();
                @NonNull final String domainName = domainId;
                @NonNull final String domainLabel = domainClass.label != null ? domainClass.label : relationName;

                @NonNull final MutableNode domainNode = new MutableNode(domainsNode, domainName);
                domainNode.setLabel(domainLabel);
        ...
        {
            @NonNull final MutableNode rangesNode = new MutableNode(relationNode, relationName + "-ranges");
            rangesNode.setLabel("ranges");
            decorateRelation(rangesNode);
            ranges.forEach(rangeClass -> {

                @NonNull final String rangeId = rangeClass.getLocalName();
                @NonNull final String rangeName = rangeId;
                @NonNull final String rangeLabel = rangeClass.label != null ? rangeClass.label : rangeName;

                @NonNull final MutableNode rangeNode = new MutableNode(rangesNode, rangeName);
                rangeNode.setLabel(rangeLabel);
        ...
        {
            @NonNull final MutableNode subPropertiesNode = new MutableNode(relationNode, relationName + "-subproperties");
            subPropertiesNode.setLabel("subproperties");
            decorateRelation(subPropertiesNode);
            subproperties.forEach(subProperty -> {

                @NonNull final String subpropertyId = subProperty.getLocalName();
                @NonNull final String subpropertyName = subpropertyId;
                @NonNull final String subpropertyLabel = subProperty.label != null ? subProperty.label : subpropertyName;

                @NonNull final MutableNode subPropertyNode = new MutableNode(subPropertiesNode, subpropertyName);
                subPropertyNode.setLabel(subpropertyLabel);
        ...
        {
            @NonNull final MutableNode inversesNode = new MutableNode(relationNode, relationName + "-inverses");
            ...
            inverses.forEach(inverseProperty -> {
                ...
                @NonNull String inverseName = inverseId;
                @NonNull String inverseLabel = inverseProperty.label != null ? inverseProperty.label : inverseName;

                @NonNull final MutableNode inverseNode = new MutableNode(inversesNode, inverseName);
                inverseNode.setLabel(inverseLabel);
    ...
    public void visitProperties(@NonNull final TreeMutableNode parentNode, @NonNull final Stream<Ontology.Property> properties)
    {
        @NonNull final List<INode> childNodes = properties //
                ...
                .map(property -> {
                    ...
                    @NonNull final String propertyId = property.getLocalName();
                    @NonNull final String propertyName = propertyId;
                    @NonNull String propertyLabel = property.label != null ? property.label : propertyName;

                    @NonNull final MutableNode propertyNode = new MutableNode(null, propertyId);
                    propertyNode.setLabel(propertyLabel);
                    propertyNode.setTarget(propertyId);
