...
public class MainFrame extends JFrame implements HyperlinkListener
{
     ...
     * @param args command line arguments
     */
    public MainFrame(@NonNull final Properties args)
    {
        // settings
        this.settings = makeSettings(Persist.loadSettings(getPersistName()), args);
    ...
    {
        return makeBrowserPane(MainFrame.class.getResource("splash/index.html"), false);
    }
     ...
     */
    @NonNull
    static public Properties makeArgs(@Nullable final String[] args)
    {
        @NonNull final Properties args2 = new Properties();
    ...
    {
        final Properties args2 = makeArgs(args);
        return makeSettings(settings, args2);
    }
     ...
     */
    @NonNull
    static public Properties makeSettings(@NonNull final Properties settings, @Nullable final Properties args)
    {
        settings.putAll(args);
        return settings;
    }
     ...
     * @param args command line arguments
     */
    private void lookup0(@NonNull final Properties args)
    {
        final String source = this.settings.getProperty("source");
        lookup(source);

        if (args.contains("source"))
            lookup(args.getProperty("source"));
    }
